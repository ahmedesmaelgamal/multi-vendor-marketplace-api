...
<form id="updateForm" method="POST" enctype="multipart/form-data" action="{{ route('products.update', $product->id) }}">
    ...
    <div class="form-group">
        ...
        <input type="file" id="testDrop" class="dropify" name="main_image"
            data-default-file="{{ $product->main_image }}" />
    </div>
    <div class="row">
        <div class="col-lg-6">
            <div class="form-group">
        ...
        <div class="col-lg-12">
            <div class="form-group">
                ...
                <textarea rows="3" name="details_at" id="details_at" class="form-control">{{ $product->details_at }}</textarea>
            </div>
        </div>
        ...
        <div class="col-lg-12">
            <div class="form-group">
                ...
                <textarea rows="3" name="details_en" id="details_en" class="form-control">{{ $product->details_en }}</textarea>
            </div>
        </div>
    </div>
    ...
    $(document).on('change', '#category_id', function() {
        ...
        if (category.length > 0) {
            ...
            $.each(subCategories, function(index) {
                ...
                $('#sub_category_id').append('<option value="' + subCategories[index].sub_category.id +
                    '">' + subCategories[index].sub_category.title_ar + '</option>')
            })
        }
    })
</script>
